...
static void finish_systolic_linebuf(void)
{
                                         ...
                                         uint32_t asynchronous)
{
    ...
    systolic_gemm32_preload(&gemm);
    systolic_gemm32_start_preloaded();
    if (asynchronous != 0u) {
        state->systolic_pending = 1u;
        return 0u;
    }
    finish_systolic_linebuf();
    return 0u;
}
                                  ...
                                  uint32_t recip_lut)
{
    ...
    const uint32_t locations = command->locations;
    (void)context;
    ...
    for (uint32_t side = 0; side < 4u; side++) {
        ...
        for (uint32_t first = 0; first < locations; first += 32u) {
            ...
            spatz_dfl_q8_to_i8(q8,
                               (int8_t *)(unsigned long)(side_destination + first),
                               (int32_t *)(unsigned long)scratch, count,
...
static uint32_t runtime_barrier(void *context)
{
    ...
    if (state != 0 && state->systolic_pending != 0u &&
        runtime_systolic_wait(context) != 0u) return 1u;
    dma_barrier();
    return 0u;
}
