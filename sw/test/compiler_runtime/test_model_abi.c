...
static void make_valid_model(uint8_t model[384])
{
    ...
    sections[3] = (nai_section_v1_t){NAI_SECTION_BINDINGS, 0, 256, 128, 32, 2, {0, 0}};
    sections[4] = (nai_section_v1_t){NAI_SECTION_QPARAMS, 0, 384, 0, 32, 0, {0, 0}};
    ...
    binding++;
    binding->direction = NAI_BINDING_L2_TEMPORARY;
    binding->data_type = NAI_DTYPE_I8;
    binding->layout = NAI_LAYOUT_NHWC;
    binding->rank = 1;
    binding->dimensions[0] = 123456;
    binding->byte_size = 123456;
...
int main(void)
{
    static const uint8_t expected_header[64] = {
        0x4e, 0x41, 0x49, 0x4d, 0x01, 0x00, 0x06, 0x00,
    ...
    assert(view.bindings->element_count == 2u);

    ((nai_model_header_v1_t *)model)->total_bytes = 383;
    ...
    make_valid_model(model);
    ((nai_binding_v1_t *)(model + 320))->index = 1u;
    assert(nai_model_open_v1(model, sizeof(model), NAI_TARGET_ID, &view) == NAI_LOADER_BAD_BINDING);
    return 0;
}
