...
module snitch_core #(
    ...
    output logic [(DATA_WIDTH/8)-1:0] obi_d_be_o,
    output logic [DATA_WIDTH-1:0]     obi_d_wdata_o,
    input  logic                      obi_d_rvalid_i,
    input  logic [DATA_WIDTH-1:0]     obi_d_rdata_i
);

    import snitch_pkg::*;

    localparam type dreq_t = `SNITCH_DATA_REQ_STRUCT(DATA_WIDTH, ADDR_WIDTH);
    localparam type drsp_t = `SNITCH_DATA_RSP_STRUCT(DATA_WIDTH);
    ...
    localparam type acc_req_t      = `SNITCH_ACC_REQ_STRUCT(DATA_WIDTH, ADDR_WIDTH);
    localparam type acc_rsp_t      = `SNITCH_ACC_RSP_STRUCT(DATA_WIDTH);
    ...
    // Protocol translation: Instruction Fetch (reqrsp -> OBI)
    assign obi_i_req_o   = inst_req.q_valid;
    assign obi_i_addr_o  = inst_req.addr;
    assign obi_i_we_o    = 1'b0; // I-fetch is always read
    assign obi_i_be_o    = '1;
    assign obi_i_wdata_o = '0;
    
    assign inst_rsp.q_ready = obi_i_gnt_i;
    assign inst_rsp.p_valid = obi_i_rvalid_i;
    assign inst_rsp.data    = obi_i_rdata_i[31:0]; // I-fetch is 32-bit
    assign inst_rsp.error   = 1'b0;

    // Protocol translation: Data Access (reqrsp -> OBI)
    assign obi_d_req_o   = data_req.q_valid;
    assign obi_d_addr_o  = data_req.q.addr;
    assign obi_d_we_o    = data_req.q.we;
    assign obi_d_be_o    = data_req.q.be;
    assign obi_d_wdata_o = data_req.q.wdata;
