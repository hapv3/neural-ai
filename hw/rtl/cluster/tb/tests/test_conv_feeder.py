...
async def boot_and_run(dut, test_file):
    ...
    fw_path = firmware_path(test_file, "sw/test/conv_feeder/conv_feeder.bin")
    assert os.path.exists(fw_path), "Run `make -C sw/test/conv_feeder` first."
...
@cocotb.test()
async def test_conv_feeder(dut):
