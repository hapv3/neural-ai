...
def golden_depthwise(height, width, channels):
    out = [0] * (height * width * c32_groups(channels) * 32)
    for oh in range(height):
        for ow in range(width):
            for channel in range(channels):
                ...
                out[c32_offset(height, width, channel, oh, ow)] = clamp_i8(acc) & 0xFF
    return out


async def run_depthwise_case(dut, fw_name, height, width, channels, report_name):
    ...
    expected = golden_depthwise(height, width, channels)
